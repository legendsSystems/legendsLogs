...
if webhooksFile == nil then
	print("^5[legendsLogs] ^1Error: Could not load webhooks file. Make sure you didn't make a typo.^0")
...
function sendWebhooks(load)
	if webhooksFile[load.channel] then
		PerformHttpRequest(
			webhooksFile[load.channel].webhook,
			function(err, text, headers)
				ServerFunc.getStatus(err, load.channel)
			end,
			"POST",
			json.encode(load.messageToDeliver),
			{
...
function GetPlayerDetails(src, config, channel)
	...
	if GetConvar("steam_webApiKey", "false") ~= "false" then
		...
		_steamURL = ""
		TriggerEvent(
			"legendsSystems:legendsLogs:errorLog",
			"You need to set a steam api key in your server.cfg for the steam identifiers to work!."
		)
	end
	...
	if config.Session or config.PlayTime and not webhooksFile[channel].Hide["PlayTime"] then
		if GetResourceState("legendsSystems_playTime") == "started" then
			playtime = exports.legendsSystems_playTime:getPlayTime(src)
			if config.Session and channel ~= "joins" and playtime ~= nil then
				playTimeArgs = SecondsToClock(playtime.Session)
				_session = "\n**Session Time:** `"
					.. string.format("%02d:%02d:%02d", playTimeArgs.hours, playTimeArgs.minutes, playTimeArgs.seconds)
					.. "`"
			else
				_session = ""
			end
			if config.PlayTime and channel ~= "joins" and playtime ~= nil then
				...
				_total = "\n**Total Time:** `"
					.. string.format(
						"%d days and %02d:%02d:%02d",
						playTimeArgs.days,
						playTimeArgs.hours,
						playTimeArgs.minutes,
						playTimeArgs.seconds
					)
					.. "`"
			else
				_total = ""
			...
			TriggerEvent("legendsSystems:legendsLogs:errorLog", "legendsSystems_playTime is not installed.")
			_total = ""
			_session = ""
		end
	else
...
function CreateLog(args)
	...
	if args["channel"] == "system" then
		message = {
			userName = "📢 legendsSystems",
			embeds = {
				{
					["color"] = 3092790,
					["author"] = {
						["name"] = "legendsSystems",
						["icon_url"] = "https://legendsSystems.com/i/DiscordIcon.png",
						["url"] = "https://discord.gg/legendsSystems",
					},
					["title"] = "📢 SYSTEM",
					["description"] = args["description"],
					["footer"] = {
						["text"] = "2020 - " .. os.date("%Y") .. " © legendsSystems • " .. os.date("%x %X %p"),
						["icon_url"] = "https://legendsSystems.com/i/DiscordIcon.png",
					},
				},
			},
			avatar_url = "https://legendsSystems.com/i/DiscordIcon.png",
		}
	...
	if webhooksFile[args.channel] then
		...
		if args.player_id then
			if webhooksFile[args.channel].logHistory then
				TriggerClientEvent(
					"legendsSystems:ClientLogStorage",
					args.player_id,
					{ Channel = args.channel, Message = args.EmbedMessage, TimeStamp = os.date("%x %X %p") }
				)
			end
		...
		print("legendsLogs: Webhooks channel not found. (" .. args.channel .. ")")
	end
end
...
function getStatus(status, channel)
	if
		status == 404
		or status == 401
			and webhooksFile[channel].webhook ~= "DISCORD_WEBHOOK"
			and webhooksFile[channel].webhook ~= ""
	then
		print(
			'^3Warn: legendsLogs webhook. Possible invalid webhook for "'
				.. channel
				.. '" webhook. Status code: '
				.. status
		)
